...
\]
% 131
\subsubsection{Calcul de $\int_{-\infty}^{+\infty}$J$(\vec{\mt{r}}$,t) dt}% c
...
Or nous avons fait l'hypothèse que $\frac{\Delta\mt{k}}{\mt{k}}$<<1 (étalement négligeable
...
Le calcul que nous venons de faire est \ul{rigoureux} moyennant les
...
\subsection{Calcul de $\sigma$ par la théorie formelle des collisions}% 3
...
\]
\subsubsection{Calcul de la section efficace}% c
...
\subsubsection{Remarques diverses}% d
...
$\beta$) \ul{Difficultés de la théorie formelle des collisions} :
...
En posant dans la formule (14), k$_\mt{i}\vec{\mt{n}}=\vec{\mt{k}}_\mt{j}$,
...
\subsubsection{Approximation de Born}% 5
...
On peut notamment faire le développement en série de Neumann-Liouville :
\[
...
\subsection{Rappels sur la représentation d'interaction (ou "point de vue de Dirac")}% 1
...
\draw [very thick] (0,0) -- (3,2.4) node [right] {$|\ \tilde{\psi}\;(-\infty)>$};
\draw [very thick] (2.9,2.0) -- (3,2.4) -- (2.6,2.4);
\draw [very thick] (0,0) -- (-3.1,2.5) node [left] {$|\ \tilde{\psi}\;(+\infty)>$};
\draw [very thick] (-3,2.1) -- (-3.1,2.5) -- (-2.7,2.5);
\draw [thick] (2.8,2.8)  arc (45:135:4);
\draw [thick] (-2.75,3.2) -- (-2.85,2.8) -- (-2.5,2.8);
\end{tikzpicture} \end{center}
...
% 146
\subsection{Définition de la matrice S. Méthode}% 2
...
D'après (47-a), (48) et (79-a), nous savons que
\[
...
\subsubsection{Limite de $<\tilde{\phi}_\mt{B }|\ \tilde{\mt{U}}\mt{(t'', t') }|\ \tilde{\phi}_\mt{A}>$}% c
